...
def get_url(url):
    ...
    for info in infos:
        ...
        release_time = info.xpath('div/div/div[1]/p[3]/text()')[0].strip()#strip()去除首尾空格
        ...
        get_info(info_url,name,star,release_time,score)#url、电影名、主演、上映时间、评分，传参给下面的函数
...
def get_info(url,name,star,time,score):
    ...
    writer.writerow([name,star,time,score,style,long_time])#电影名、主演、上映时间、评分、类型、时长

if __name__ == '__main__':
    fp = open('e:\maoyan.csv','w',encoding='gbk',newline='')
